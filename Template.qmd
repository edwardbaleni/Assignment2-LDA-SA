...
Sentiment analysis is a technique to programmatically assess the emotional content of a text (Silge & Robinson, 2017). It provides a way, through the use of sentiment lexicons, to understand the attitudes and opinions expressed in a piece of text or speech and identify if they are positive or negative (Silge & Robinson, 2017). Sentiment analysis is seen as a classification method as it classifies documents into positive or negative categories or more sophisticated classes such as joy, disgust or anger, depending on the specific sentiment lexicon used in the analysis (Medhat et al., 2014). Due to the increasingly large amounts of text data around today, sentiment analysis has become more popular and necessary as a tool for document classification and arrangement as the task has become manually impractical (Medhat et al., 2014). There are a number of application areas for sentiment analysis including customer feedback and reviews, social media monitoring, market research and political analysis. In the context of the SONA data, the sentiment analysis can be seen as a form of political analysis and a way in which to describe how the country progressed in terms of governance and economy from 1994 to 2023.

#### ***Topic Modelling***

Topic Modelling is a unsupervised classification technique for grouping documents according to topic or theme (Silge & Robinson, 2017). The particular topic modelling method that is popular in the literature is Latent Dirichlet Allocation (Silge & Robinson, 2017). The key ideas of this method is to consider documents as a collection of topics and topics as a collection of words (Silge & Robinson, 2017). Topic modelling, allows one to categorize documents into broad themes and calculate the proportions of each topic or theme that appear in a document. Miranda and Bringula (2021) used Topic Modelling to analsye the SONA speeches of Philippine presidents. Through the use of Latent Dirichlet Allocation, they were able to identify three major topics each linked with concerns within the country. In general, a topic can be labelled by assessing the sentences that contain the words in that topic. Miranda & Bringula (2021) emphasise that this process needs to be thoroughly discussed and a general consensus has to be reached on what labels best represent each topic. This gives an indication that selecting the labels for topics is not always an easy task and may in some instances not represent perfectly the true underlying topic found by the LDA analysis.
...
A separate analysis is done on bigrams in order to consider negation of words, for example 'improvement' vs 'no improvement', and to assess how frequently negation occurred. In order to perform this analysis the unnest_tokens() function was used specifying the token argument to 'ngrams' and setting 'n' to 2. The same cleaning was done in order to remove numbers and punctuation marks before the speeches were tokenized in this way. In terms of stop word removal, stop words were removed except for those stop words that are negation words. The qdapDictionary::negation.words dictionary was used to identify negation words and remove them from the stop words before removing stop words from the analysis.
...
The negation words used here have been drawn from the dictionary provided by (Rinker, T. W., 2013). It is important to note that this list of negation words is not exhaustive, and this limitation could affect the outcomes of the analysis.
...
The speeches from 2020 to 2023 decreased significantly in net positive sentiment which makes sense due to the COVID-19 pandemic and its economic impact on the country. However these SONA speeches were still overall more positive than negative but not as positive as Ramaphosa first official SONA as president (2019).
...
During Zuma's presidency, the negative sentiment that was expressed was undeniably negative. It seemed to be a period had greater amounts of corruption as the word corruption appears frequently in the bigrams. Apart from the attempt to combat corruption, the sentiment in Zuma's speeches does not seem to have any double meaning. The speaker seems to speak on the financial and economic crisis present in the country at the time.
...
Figure C, D and E in the [Appendix](Appendix.qmd) show the use of negations in bigrams to identify if any positive or negative words are frequently negated. It can be seen that negation words do not appear frequently in these speeches or in front of other words. This is an important observation particularly for both the positive and negative individual words identified above. The fact that the negation of these positive and negative sentiments happens infrequently indicates that on the whole the positive and negative words identified are used within the correct context and are not negated.

### *Topic Modelling*

Having considered the sentiment analysis results of each speech, the topic modelling results are now presented.

As mentioned in the methods section, in order to specify an LDA model, one has to specify the number of topics one could expect to see in a document. Having selected this number of topics, the LDA will produces two outputs beta, $\beta$, and gamma, $\gamma$. Where $\beta$ is a parameter that gives the probability of a topic generating a particular word and $\gamma$ gives the proportions of topics in a document (Durbach, I., 2023).
...
The plot for Mbeki shows a much larger amount of change in topics between different speeches. In each of his speeches the dominating topic seems to shift. In his first and third speeches he seems to talk on politics and administration. In his second and sixth speech his dominating topic is that of social development. His fifth, eighth and ninth speeches deal with government policy. His fourth and seventh speech have a combination of ideas these different topics. In his last years of presidency, the main theme of his speeches was that of social development.
...
Durbach, I. 2023. "Topic Modelling." Data Science for Industry. 07-textmining-05.html. Statistical Sciences Department, University of Cape Town.
...
[GitHub Repo](https://github.com/edwardbaleni/Assignment2-LDA-SA)

https://github.com/edwardbaleni/Assignment2-LDA-SA

[GitHub Pages Website](https://edwardbaleni.github.io/Assignment2-LDA-SA/)

https://edwardbaleni.github.io/Assignment2-LDA-SA/
